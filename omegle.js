...
Omegle.prototype.start = function (callback, proxyInfo) {
    ...
    this.getAccessCode().then(() => {
        ...
        console.log('Had an error while getting an access code.');
        console.log(e);
    });
};
...
Omegle.prototype.getAccessCode = async function () {
    // dont get more than one access code
    if(!!this.accessCode) return;

    // pick a server
    const selectedAntiNudeServer = antiNudeServerList[Math.floor(Math.random() * antiNudeServerList.length)];

    // Grab a code
    const res = await axios.post('https://' + selectedAntiNudeServer + '/check');
...
(function () {
    ...
    om.getStatus(function (status) {
        ...
        // Get the anti nude servers
        antiNudeServerList = status.antinudeservers;
